...
public class OpenArgsUI extends JFrame implements Progreso {
  ...
  public OpenArgsUI() throws IOException {
    ...
    // Si inicio OpenArgsUI sin argumentos formateo para que escriba igual en un
    // log
    this.logger = Logger.getLogger(OpenArgs.class.getName());
   ...
   * @throws IOException
   */
  public OpenArgsUI(String uuid, String tokenApi, Logger logger, String rutaApi)
      throws IOException {
    super("Firmador");
  ...
  private void initFrame() throws IOException {
    ...
    add(etiqueta,
        new GBC(0, 0, 2, 1).setAnchor(java.awt.GridBagConstraints.NORTH)
            .setFill(java.awt.GridBagConstraints.NONE).setInsets(10)
            .setWeight(cien, cien));
    add(uuidLabel,
        new GBC(0, 1).setAnchor(java.awt.GridBagConstraints.EAST)
            .setFill(java.awt.GridBagConstraints.NONE).setInsets(5)
            .setWeight(cien, cien));
    add(uuidField,
        new GBC(1, 1).setAnchor(java.awt.GridBagConstraints.WEST)
            .setFill(java.awt.GridBagConstraints.NONE).setInsets(5)
            .setWeight(cien, cien));
    add(pwdLabel,
        new GBC(0, 2).setAnchor(java.awt.GridBagConstraints.EAST)
            .setFill(java.awt.GridBagConstraints.NONE).setInsets(5)
            .setWeight(cien, cien));
    add(pwdField,
        new GBC(1, 2).setAnchor(java.awt.GridBagConstraints.WEST)
            .setFill(java.awt.GridBagConstraints.NONE).setInsets(5)
            .setWeight(cien, cien));

    // la línea de botones va en un panel propio
    JPanel panelFirmaSalir = new JPanel();
    panelFirmaSalir.setLayout(new FlowLayout(FlowLayout.CENTER));
    panelFirmaSalir.add(firmarBtn);
    panelFirmaSalir.add(salirBtn);
    add(panelFirmaSalir,
        new GBC(0, 3, 2, 1).setAnchor(java.awt.GridBagConstraints.NORTH)
            .setInsets(5).setWeight(cien, cien));

    add(statusLabel,
        new GBC(0, 4, 2, 1).setAnchor(java.awt.GridBagConstraints.NORTH)
            .setFill(java.awt.GridBagConstraints.NONE).setInsets(20)
            .setWeight(cien, cien));
  ...
  private void firmar(ActionEvent e) {
    ...
    try {
      response = Unirest.post(urlRecuperarFirmar)
          ...
          .header("authorization", "Bearer " + tokenApi).body(cadenaJson)
          .asJson().ifFailure(response2 -> {
            JOptionPane.showMessageDialog(this,
                "Hubo un error al solicitar los archivos,  "
      ...
      JOptionPane.showMessageDialog(this,
          "Error al conectar con " + urlRecuperarFirmar);
    ...
    try {
      ...
      JOptionPane.showMessageDialog(this,
          "Error al crear estructura de directorios intermedios temporales");

      ex.printStackTrace();
      setStatus(mensajeOriginal);
      logger.log(Level.SEVERE,
          "Error al crear estructura de directorios intermedios ", ex);
      return;
    }
    ...
    for (cont = 0; cont < data.data.length; cont++) {
      ...
      decodedFile = Base64.getDecoder().decode(data.data[cont].arch);
      ...
      listaSrcDest.add(new SrcDest(destinationFile.toString(),
          destinationFileSigned.toString()));

      try {
        Files.write(destinationFile, decodedFile);
      } catch (Exception | Error ex) {
        JOptionPane.showMessageDialog(this,
            "Error al escribir en destino temporal");
        ex.printStackTrace();
        logger.log(Level.SEVERE, "Error al escribir en destino temporal "
            + destinationFile.toString(), ex);
      }
    ...
    // usando la libreria dinamica espcecifica povista por el proveedor del
    // token
    UsbToken tokenUnix;
    try {
      // "/usr/lib/x64-athena/libASEP11.so" para linux
      tokenUnix = new UsbToken(libreria);
    ...
    if (this.pwdField.getPassword().length > 0) {
      ...
      try {
        ...
        etiqueta.setText(x509.getSubjectX500Principal().getName().split("=")[1]
            .split(",")[0]);
        // System.out.println("getName: " +
        // x509.getSubjectX500Principal().getName().split("=")[1].split(",")[0]);
        etiqueta.paintImmediately(etiqueta.getVisibleRect());
      } catch (Exception ex1) {
    ...
    // al terminar de firmar cargo todos los archivos firmados para subir al
    ...
    for (cont = 0; cont < data.data.length; cont++) {
      try {
        // "/tmp/1_signed.pdf"
        readedFile = Files
            .readAllBytes(Paths.get(temp, uuid, nombresArchivosFirmados[cont]));
        String encodedFile = Base64.getEncoder().encodeToString(readedFile);
        data.data[cont].arch = encodedFile; // data.data[0]=encodedFile;

        // quitar el .pdf devuelvo como nombre de archivo el original
        nombreArchivoFirmado = nombresArchivos[cont];
        nombreArchivoFirmado = nombreArchivoFirmado.substring(0,
            nombreArchivoFirmado.length() - 4);
        ...
        data.data[cont] = null;
        logger.log(Level.SEVERE,
            "Error al leer archivo firmado " + nombresArchivosFirmados[cont],
            ex);
      }
    }
    ...
    try {
      HttpResponse<String> response1 = Unirest.post(urlGuardarFirmados)
          .header("accept", APPLICATION_JSON)
          .header("authorization", "Bearer " + tokenApi).body(cadenaJson1)
          .asString().ifFailure(response2 -> {
            JOptionPane.showMessageDialog(this,
                "Hubo un error al guardar los archivos firmados,  "
                    + response2.getStatusText());
      ...
      JOptionPane.showMessageDialog(this,
          "Error al conectar con " + urlGuardarFirmados);
      
      ex.printStackTrace();
      setStatus(mensajeOriginal);
      logger.log(Level.SEVERE, "Error al conectar con  " + urlGuardarFirmados,
          ex);
    ...
    try {
      response = Unirest.post(urlConfirmarFinalizacion)
          ...
          .asJson().ifFailure(response2 -> {
            JOptionPane.showMessageDialog(this,
                "Hubo un error al confirmar la finalización,  "
                    + response2.getStatusText());
      ...
      JOptionPane.showMessageDialog(this,
          "Error al conectar con " + urlConfirmarFinalizacion);
  ...
  /**
   * Implementación de la interfaz Progreso para indicar mensajes durante las
   * acciones.
   */
