...
public class UsbToken {
	...
	private SunPKCS11 buildProvider(String tokenLibraryPath) {
		try {
			...
			return new SunPKCS11(tmpConfigFile.getAbsolutePath()); //si va con java 11 usar las 3 líneas de abajo en vez
	...
	/**
	 * Firma los PDFs
	 * @param srcDest  (source path of the Pdf to be signed and destination path of the signed Pdf)
	 * @param frame
	 * @param pin
	 * @param x509
	 */
	public void signPdfs(List<SrcDest> srcDest, Progreso frame, char[] pin, X509Certificate x509 ) {
		...
		try { 
            ...
            String l2text = x509.getSubjectX500Principal().getName().split("=")[1].split(",")[0];
            ...
            int cantFirmas;
            cantFirmas=0;
            float x,y,x1,y1,alto,ancho,cm,desplazamiento;
            float a4Ancho = 595;
            float a4Alto = 842;
			...
			for (SrcDest path : srcDest) {
	            ...
	            PdfSigner signer = new PdfSigner(reader, new FileOutputStream(path.dest()), sp);
	            ...
	            altoPagina1 = signer.getDocument().getFirstPage().getPageSize().getHeight()-150; //resto 150 porque si no me quedaba muy arriba y no se veía
                ...
                //las coordenadas arrancan desde abajo por eso tengo que invertir y con y1
                //Rectangle rect = new Rectangle(x,altoPagina1-y1-desplazamiento,x1,altoPagina1-y-desplazamiento); //esto es si se usa setBox
                Rectangle rect = new Rectangle(x,altoPagina1-y-desplazamiento,ancho,alto);
			...
			if (rootCause instanceof sun.security.pkcs11.wrapper.PKCS11Exception) {
				...
				if ("CKR_PIN_LOCKED".equals(rootCause.getMessage())) {
					throw new RuntimeException("Su Token esta bloqueado por intentos incorrectos", e);
				}
			}	
			throw new RuntimeException("Ups, something went wrong... ", e);
		} finally {
	...
	private void resetProvider()  {
		...
		try {
			...
			throw new RuntimeException("Ups, something went wrong logging out... ", e);
		}
	}
